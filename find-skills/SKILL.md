...
2. If `skillhub-canonical` is unavailable but `skillhub` exists, use `skillhub --dir ~/.agents/skills install <slug>` and then link/commit manually.
3. If no `skillhub` candidate exists, use `clawhub install <slug>` only when the user explicitly wants the fallback source.
